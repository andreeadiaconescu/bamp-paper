function bamp_hgf_binary_plotTraj(r,sim_bamp,sim_actual_y,options)
...
probabilityAdvice=options.task.probabilities;
nTrials = options.task.TrialsperBlock;
...
hold all;
probs=options.task.TrialbyTrialprob;
plot(ts(2:end),probs','Color',[0.5 0.5 0.5], 'LineWidth', 2);
plot(0, tapas_sgm(r.p_prc.mu_0(2), 1), 'or', 'LineWidth', 2); % prior
plot(ts(2:end), r.u(:,1), '.', 'Color', [0 0.6 0]); % inputs
plot(ts(2:end), r.traj.sahat(:,1), 'k', 'LineWidth', 2) % implied learning rate
if (ploty == true) && ~isempty(find(strcmp(fieldnames(r),'y'))) && ~isempty(r.y)
    ...
    plot(ts(1:end-1), r.cue_advice_space, 'Color', 'b','LineStyle','-.');% plot response probability for response 1
    title(['Response y (orange), input u (green), learning rate (fine black), and posterior expectation of input s(\mu_2) ', ...
        '(red) for \zeta=', num2str(r.p_obs.ze1), ', \kappa=', ...
